...
int main() {
    FILE *fp;
    float *fft_output;
    int N = 8192, i, peak_index; // N is known to be 8192
    float max_value;

    // Open the file
    fp = fopen("fft_output.txt", "r");
    if (fp == NULL) {
    ...
    fft_output = (float *)malloc(N * sizeof(float));
    if (fft_output == NULL) {
    ...
    for (i = 0; i < N; i++) {
        ...
        fft_output[i] = sqrt(real * real + imag * imag); // Magnitude
    }
    ...
    float sample_rate = 44100; // Sample rate, change accordingly if your sample rate is different
